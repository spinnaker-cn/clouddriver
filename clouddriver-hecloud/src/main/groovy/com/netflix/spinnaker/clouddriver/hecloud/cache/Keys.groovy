package com.netflix.spinnaker.clouddriver.hecloud.cache
...
class Keys implements KeyParser {
  ...
  static Map<String, String> parse(String key) {
    ...
    if (parts.length < 2 || parts[0] != HeCloudProvider.ID) {
      return null
    }
  ...
  static String getApplicationKey(String application) {
    "$HeCloudProvider.ID:$Namespace.APPLICATIONS:${application.toLowerCase()}"
  }

  static String getClusterKey(String clusterName, String application, String account) {
    "$HeCloudProvider.ID:$Namespace.CLUSTERS:$account:${application?.toLowerCase()}:$clusterName"
  }
  ...
  static String getTargetHealthKey(String loadBalancerId, String listenerId, String poolId, String instanceId, String account, String region) {
    "$HeCloudProvider.ID:$Namespace.HEALTH_CHECKS:$account:$region:$loadBalancerId:$listenerId:$poolId:$instanceId"
  }
}
