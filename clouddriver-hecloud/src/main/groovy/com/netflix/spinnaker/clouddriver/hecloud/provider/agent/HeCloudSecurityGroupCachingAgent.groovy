...
class HeCloudSecurityGroupCachingAgent implements CachingAgent, OnDemandAgent, AccountAware {
  ...
  HeCloudSecurityGroupCachingAgent(HeCloudNamedAccountCredentials creds,
    ...
    this.metricsSupport = new OnDemandMetricsSupport(registry, this, "${HeCloudProvider.ID}:${OnDemandAgent.OnDemandType.SecurityGroup}")
  }
  ...
  boolean handles(OnDemandAgent.OnDemandType type, String cloudProvider) {
    type == OnDemandAgent.OnDemandType.SecurityGroup && cloudProvider == HeCloudProvider.ID
  }

  @Override
  OnDemandAgent.OnDemandResult handle(ProviderCache providerCache, Map<String, ? extends Object> data) {
    log.info("Enter HeCloudSecurityGroupCachingAgent handle, params = ${data}")
    if (!data.containsKey("securityGroupId") ||
      !data.containsKey("account") ||
      !data.containsKey("region") ||
      accountName != data.account ||
      region != data.region) {
      log.info("HeCloudSecurityGroupCachingAgent: input params error!")
      return null
    }

    HeCloudSecurityGroupDescription updatedSecurityGroup = null
    HeCloudSecurityGroupDescription evictedSecurityGroup = null
    ...
    if (!updatedSecurityGroup) {
      log.info("HeCloudSecurityGroupCachingAgent: Can not find securityGroup ${securityGroupId} in ${region}")
      return null
    }
    ...
    def cacheResult = metricsSupport.transformData {
      if (updatedSecurityGroup) {
        ...
        evictedSecurityGroup = new HeCloudSecurityGroupDescription(
    ...
    log.info("HeCloudSecurityGroupCachingAgent: onDemand cache refresh (data: ${data}, evictions: ${evictions})")
  ...
  CacheResult loadData(ProviderCache providerCache) {
    log.info("Enter HeCloudSecurityGroupCachingAgent loadData in ${agentType}")
  ...
  private Set<HeCloudSecurityGroupDescription> loadSecurityGroupAll() {
    ...
    def securityGroupDescriptionSet =  securityGroupSet.collect {
      def securityGroupDesc = new HeCloudSecurityGroupDescription()
      ...
      securityGroupDesc.inRules = ingressRules.collect { ingress ->
        def inRule = new HeCloudSecurityGroupRule()
      ...
      securityGroupDesc.outRules = egressRules.collect { egress ->
        def outRule = new HeCloudSecurityGroupRule()
  ...
  private HeCloudSecurityGroupDescription loadSecurityGroupById(String securityGroupId) {
    HeCloudVirtualPrivateCloudClient vpcClient = new HeCloudVirtualPrivateCloudClient(
      credentials.credentials.accessKeyId,
      credentials.credentials.accessSecretKey,
      region
  ...
  private CacheResult buildCacheResult(ProviderCache providerCache,
                                       Collection<HeCloudSecurityGroupDescription> securityGroups,
                                       long lastReadTime,
                                       HeCloudSecurityGroupDescription updatedSecurityGroup,
                                       HeCloudSecurityGroupDescription evictedSecurityGroup) {
    if (securityGroups) {
      ...
      securityGroups.each { HeCloudSecurityGroupDescription item ->
        HeCloudSecurityGroupDescription securityGroup = item
        ...
        if (onDemandSG) {
          if (onDemandSG.attributes.cachedTime > securityGroup.lastReadTime) {
            ...
            securityGroup = objectMapper.readValue(onDemandSG.attributes.securityGroup as String, HeCloudSecurityGroupDescription)
          } else {
            // Found a Security Group that has been deleted since last time was read from HeCloud cloud
            securityGroup = null
          }
  ...
  private CacheData buildCacheData(HeCloudSecurityGroupDescription securityGroup) {
  ...
  private Boolean updateCache(ProviderCache providerCache, HeCloudSecurityGroupDescription securityGroup, String onDemandCacheType) {
